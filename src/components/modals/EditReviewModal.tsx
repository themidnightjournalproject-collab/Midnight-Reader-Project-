...
import { Genre, Suspect, ClueItem } from '../../types';
import { 
  Plus, 
  Trash2, 
  Edit3, 
...
export const EditReviewModal: React.FC = () => {
  const { 
    editingReview, 
    setEditingReview, 
    updateReview, 
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'review' | 'evidence' | 'suspects' | 'spoilers' | 'metrics'>('basic');
  ...
  // Interactive Suspects & Clues Lineup
  ...
  // Engagement & Poll Metrics
  const [likesCount, setLikesCount] = useState<number>(1);
  const [pollSolvedBeforeTwist, setPollSolvedBeforeTwist] = useState<number>(0);
  const [pollFooledCompletely, setPollFooledCompletely] = useState<number>(0);
  const [pollSawItComingMidway, setPollSawItComingMidway] = useState<number>(0);
  const [pollJawDroppedEnding, setPollJawDroppedEnding] = useState<number>(0);

  useEffect(() => {
    if (editingReview) {
      setCaseNumber(editingReview.caseNumber || '001');
      setTitle(editingReview.title || '');
      setAuthor(editingReview.author || '');
      setCoverImage(editingReview.coverImage || '');
      setGenre(editingReview.genre || 'Psychological Thriller');
      setDateRead(editingReview.dateRead || new Date().toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }));
      setDateStarted(editingReview.dateStarted || '');
      setDateFinished(editingReview.dateFinished || '');
      setYearRead(editingReview.yearRead || new Date().getFullYear());
      setPages(editingReview.pages || 300);
      setRating(editingReview.rating || 5);
      setStatus(editingReview.status || 'CLOSED');
      setReadingFormat(editingReview.readingFormat || 'Paperback');
      setCorrectGuess(editingReview.correctGuess ?? true);

      setLeadQuote(editingReview.leadQuote || '');
      setSummary(editingReview.summary || '');
      setReviewText(editingReview.reviewText || '');
      setWhatIThought(editingReview.whatIThought || '');
      setWhatActuallyHappened(editingReview.whatActuallyHappened || '');
      setFavoriteMoment(editingReview.favoriteMoment || '');
      setFinalVerdict(editingReview.finalVerdict || '');
      setRecommendedFor(editingReview.recommendedFor || 'Fans of gripping psychological suspense');
      setSelectedTropes(editingReview.tropes || ['Unreliable Narrator']);

      setAtmosphereRating(editingReview.atmosphereRating || 5);
      setPacingRating(editingReview.pacingRating || 5);
      setTwistRating(editingReview.twistExecutionRating || 5);
      setParanoiaRating(editingReview.paranoiaRating || 4);
      setUnreliableRating(editingReview.unreliableNarratorRating || 4);
      setTwistsCount(editingReview.twistsCount || 2);

      setEvidenceBullets(editingReview.evidenceBullets || [
        'Initial crime scene inconsistencies noted by investigator',
        'Conflicting testimonies recorded during initial interview'
      ]);
      setMarginaliaNotes(editingReview.marginaliaNotes || [
        'Watch page numbers closely—the narrator is intentionally omitting details!'
      ]);

      setSuspects(editingReview.suspects || []);
      setClues(editingReview.clues || []);
      setSpoilerEvidence(editingReview.spoilerEvidence || '');
      setSpoilerCulpritReveal(editingReview.spoilerCulpritReveal || '');

      setLikesCount(editingReview.likes || 1);
      if (editingReview.readerPollVotes) {
        setPollSolvedBeforeTwist(editingReview.readerPollVotes.solvedBeforeTwist || 0);
        setPollFooledCompletely(editingReview.readerPollVotes.fooledCompletely || 0);
        setPollSawItComingMidway(editingReview.readerPollVotes.sawItComingMidway || 0);
        setPollJawDroppedEnding(editingReview.readerPollVotes.jawDroppedEnding || 0);
      }
    }
  }, [editingReview]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!title.trim() || !author.trim()) {
      alert('Please specify at least a Book Title and Author.');
      return;
    }

    const cleanCaseNumber = caseNumber.trim() || editingReview.caseNumber || '001';
    ...
    updateReview(editingReview.id, {
      ...
      genre,
      dateRead: dateRead.trim() || editingReview.dateRead,
      ...
      readerPollVotes: {
        solvedBeforeTwist: Number(pollSolvedBeforeTwist) || 0,
        fooledCompletely: Number(pollFooledCompletely) || 0,
        sawItComingMidway: Number(pollSawItComingMidway) || 0,
        jawDroppedEnding: Number(pollJawDroppedEnding) || 0,
      }
    });
  ...
  const handleDelete = () => {
    if (window.confirm(`Are you sure you want to permanently delete Case #${editingReview.caseNumber}: "${editingReview.title}"? This cannot be undone.`)) {
    ...
    <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-white border-4 border-black w-full max-w-4xl max-h-[94vh] overflow-y-auto p-4 sm:p-7 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] my-4">
        ...
        <form onSubmit={handleSave} className="space-y-5 text-xs font-sans">
            ...
            <div className="bg-[#faf9f5] p-5 border-2 border-black space-y-4">
              ...
              <div className="bg-white border-2 border-[#8b0000] p-3 space-y-1">
                ...
                <span className="text-[10px] text-[#737373] italic">
                  Change this number anytime to reorder or reassign this dossier’s archive designation.
                </span>
              </div>
          ...
          {activeTab === 'review' && (
            <div className="bg-[#faf9f5] p-5 border-2 border-black space-y-5">
              <div className="border-b border-black pb-2">
                ...
                <p className="font-serif italic text-xs text-[#4a4a4a] mt-0.5">
                  Update your in-depth review text, theories, final verdicts, tropes, and forensic sliders.
                </p>
              </div>
            ...
            <div className="bg-[#fff8f8] p-5 border-2 border-[#8b0000] space-y-4">
              <div className="flex items-center gap-2 text-[#8b0000] border-b border-[#8b0000] pb-2">
                ...
                <div>
                  ...
                  <p className="font-serif italic text-xs text-[#4a4a4a] mt-0.5">
                    This section remains sealed behind an interactive <strong>"CLASSIFIED DOSSIER"</strong> stamp for readers on the live website until they click to reveal.
                  </p>
                </div>
              </div>
          ...
          {/* TAB 6: ENGAGEMENT METRICS & READER POLLS */}
          {activeTab === 'metrics' && (
            <div className="bg-[#faf9f5] p-5 border-2 border-black space-y-4">
              <div className="border-b border-black pb-2">
                <h4 className="font-bold text-[#1a1a1a] uppercase tracking-wider text-xs">
                  Reader Engagement, Endorsements & Poll Counts
                </h4>
                <p className="font-serif italic text-xs text-[#4a4a4a] mt-0.5">
                  Adjust community likes and reader poll vote counters for this specific case dossier.
                </p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-white border border-black p-3 space-y-1">
                  <label className="block text-[#1a1a1a] font-bold uppercase text-[10px] flex items-center gap-1">
                    <ThumbsUp size={12} className="text-[#8b0000]" />
                  ...
                  <input
                    ...
                    className="w-full bg-[#faf9f5] border border-black p-2 text-sm font-mono font-bold text-[#1a1a1a]"
                  />
                </div>

                <div className="bg-white border border-black p-3 space-y-1">
                  <label className="block text-[#1a1a1a] font-bold uppercase text-[10px]">
                    Poll: Solved Before The Twist
                  </label>
                  <input
                    type="number"
                    min={0}
                    value={pollSolvedBeforeTwist}
                    onChange={(e) => setPollSolvedBeforeTwist(Number(e.target.value))}
                    className="w-full bg-[#faf9f5] border border-black p-2 text-sm font-mono font-bold text-[#1a1a1a]"
                  />
                </div>
                ...
                <div className="bg-white border border-black p-3 space-y-1">
                  <label className="block text-[#1a1a1a] font-bold uppercase text-[10px]">
                    Poll: Fooled Completely (Red Herring Victim)
                  </label>
                  <input
                    type="number"
                    min={0}
                    value={pollFooledCompletely}
                    onChange={(e) => setPollFooledCompletely(Number(e.target.value))}
                    className="w-full bg-[#faf9f5] border border-black p-2 text-sm font-mono font-bold text-[#1a1a1a]"
                  />
                </div>
                ...
                <div className="bg-white border border-black p-3 space-y-1">
                  <label className="block text-[#1a1a1a] font-bold uppercase text-[10px]">
                    Poll: Saw It Coming Midway
                  </label>
                  <input
                    type="number"
                    min={0}
                    value={pollSawItComingMidway}
                    onChange={(e) => setPollSawItComingMidway(Number(e.target.value))}
                    className="w-full bg-[#faf9f5] border border-black p-2 text-sm font-mono font-bold text-[#1a1a1a]"
                  />
                </div>

                <div className="bg-white border border-black p-3 space-y-1 sm:col-span-2">
                  <label className="block text-[#1a1a1a] font-bold uppercase text-[10px]">
                    Poll: Jaw-Dropped Ending Shock
                  </label>
                  <input
                    type="number"
                    min={0}
                    value={pollJawDroppedEnding}
                    onChange={(e) => setPollJawDroppedEnding(Number(e.target.value))}
                    className="w-full bg-[#faf9f5] border border-black p-2 text-sm font-mono font-bold text-[#1a1a1a]"
                  />
                </div>
              </div>
          ...
          {/* Bottom Actions Bar */}
          <div className="flex flex-wrap items-center justify-between border-t-2 border-black pt-4 gap-3">
            <button
              type="button"
              onClick={handleDelete}
              className="flex items-center gap-1.5 text-[#8b0000] hover:bg-[#ffefef] px-3.5 py-2 border-2 border-[#8b0000] text-xs font-sans font-bold uppercase transition-colors"
            >
              <Trash2 size={13} />
              <span>Delete This Case File</span>
            </button>
